...
g <- ggplot(data = home_away, aes(x = LOCATION, y = PERCENTAGE, fill = LOCATION)) + geom_bar(stat = 'identity')
g
...
w <- ggplot(data = wins, aes(LOCATION, fill = factor(W)) )+ geom_bar()
w
```
...
na_cols <- NULL
columns <- names(nba_shots)

for (col in columns){

  if(any(is.na(nba_shots[, col]))){
    
    na_cols[col] <- sum(is.na(nba_shots[, col]))
...
The next step is to one-hot code the factors to accomodate the requirement of xgboost.
